...
import { validateEntityJson } from '@/lib/entityJson/validators';
import { logJsonEdit } from '@/lib/entityJson/audit';
...
export function EntityJsonEditor({
  ...
  const exampleJson = entitySchema?.example || {};

  // Track if user is actively editing to prevent auto-updates
  const [isUserEditing, setIsUserEditing] = useState(false);
  const previousContentRef = useRef<string>('');

  // Initialize JSON value from entity value
  useEffect(() => {
    try {
      ...
      setParseError('Failed to parse entity content');
      setJsonValue('{}');
  ...
  const validateDebounced = useCallback(
    debounce((value: string) => {
      ...
      try {
        ...
        const result = entitySchema
          ? validateEntityJson(parsed, entitySchema.schema)
          : { valid: true, errors: [] };
    ...
    [entitySchema, onValidationChange]
  );
  ...
  const handleExport = () => {
    try {
      ...
      // Sanitize filename
      const name = fileName || entityType;
      const id = entityId ? `-${entityId.slice(0, 8)}` : '';
      link.download = `${name}${id}.json`;
    ...
    if (validationResult.valid && !parseError && jsonValue && isUserEditing) {
      const timeoutId = setTimeout(() => {
        try {
          ...
          // Log JSON edit (debounced to avoid excessive logging)
          if (entityId) {
            logJsonEdit(entityType, entityId, fileName, parsed).catch(console.error);
          }
        } catch {
  ...
  }, [validationResult.valid, parseError, jsonValue, onChange, isUserEditing, entityId, entityType, fileName]);
                  ...
                  <div className="space-y-4">
                    <div>
                      <p className="text-xs text-foreground/80 mb-3 leading-relaxed">
                        This is an example JSON structure for {entityType}. Use this as a reference when writing or uploading JSON data.
                      </p>
                      {entitySchema.description && (
                        <p className="text-xs text-foreground/60 mb-2 italic">
                          {entitySchema.description}
                        </p>
                      )}
